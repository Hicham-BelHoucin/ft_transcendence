...
const addFriend = async (senderId: number, receiverId: number) => {
	try {
		await axios.post(
			`${process.env.BACK_END_URL}api/users/add-friend`,
			{
				senderId: senderId,
...
const acceptFriend = async (id: number) => {
	try {
		await axios.get(`${process.env.BACK_END_URL}api/users/accept-friend/${id}`, {
			withCredentials: true,
		});
...
const cancelFriend = async (id: number) => {
	try {
		await axios.delete(`${process.env.BACK_END_URL}api/users/remove-friend/${id}`, {
			withCredentials: true,
		});
...
export { addFriend, cancelFriend, acceptFriend, BlockUser, UnBlockUser, isBlocked };

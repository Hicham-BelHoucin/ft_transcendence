...
const Channel: React.FC<ChannelProps> = ({
	...
	const MessageDate = () => {
		...
		if (timeDifference >= 86400) {
			...
			else if (days > 365) return `${Math.floor(days / 365)}y ago`;
			else if (days > 30) return `${Math.floor(days / 30)}m ago`;
			else if (days > 7) return `${Math.floor(days / 7)}w ago`;
	...
	return (
		<div
			className={twMerge(
				"relative flex items-center justify-between gap-[10px] w-full px-4 cursor-pointer hover:bg-tertiary-700 rounded-xl py-2",
		...
		>
			<div className="flex items-center gap-2">
				<Avatar src={avatar!} alt="" status={userStatus ? "ONLINE" : undefined} />
			...
			<div className="flex items-right flex-col text-black text-sm justify-end">
				<span
					className={twMerge(
						"p-0 w-14",
						unread ? "text-primary-500" : "text-secondary-300"
					)}
				>
					{MessageDate()}
				</span>
			...
			{showMenu && (
				<RightClickMenu className="right-20 top-10">
					<RightClickMenuItem
						onClick={(e: any) => {
							pinned ? unpinChannel(e) : pinChannel(e);
						}}
					>
						<Pin />
					...
					<RightClickMenuItem
						onClick={(e) => {
							muted ? unmuteChannel(e) : muteChannel(e);
						}}
					>
						<VolumeX />

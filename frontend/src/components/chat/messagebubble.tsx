...
const MessageBubble: React.FC<ChannelProps> = ({ className, setOpen, setCurrentChannel, currentChannel, channelMember, messages, inputRef }: ChannelProps) => {
  ...
  const [blocking, setBlocking] = useState<any[]>(user?.blocking?.map((blocking: any) => { return blocking.blockerId }) as any[]);
  const [blocked, setBlocked] = useState<any[]>(user?.blockers?.map((blocker: any) => { return blocker.blockingId }) as any[]);
